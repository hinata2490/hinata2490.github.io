# コスメの事情 - TikTok自動化プロジェクト

化粧品が擬人化されてキレながら使い方を解説するTikTokチャンネル。

- **アカウント名**: コスメの事情（@cosme_no_jijou）
- **ジャンル**: 美容コスメ擬人化エンタメ
- **ターゲット**: 18〜28歳女性、美容に興味あるけど詳しくない層
- **動画尺**: 30〜45秒（フォロワー1万超えで1分超に切替）
- **投稿頻度**: 週5本

## ドキュメント

- [CLAUDE.md](CLAUDE.md) - Claude Code 用プロジェクトコンテキスト（最初に読む）
- [docs/strategy.md](docs/strategy.md) - 戦略（収益・競合・差別化・ロジック）
- [docs/production.md](docs/production.md) - 制作（仕様・ワークフロー・Kling運用・法務）
- [docs/account-setup.md](docs/account-setup.md) - アカウント開設手順
- [docs/shadowban-incident.md](docs/shadowban-incident.md) - **ep01投稿後0再生の原因分析と対策（必読）**
- [episodes/ep01-hatomugi.md](episodes/ep01-hatomugi.md) - 1本目: 台本とチェックリスト
- [episodes/ep01-hatomugi-workflow.md](episodes/ep01-hatomugi-workflow.md) - **1本目: 制作ワークフロー（コピペ実行用）**

## 今日のゴール: 1本目をTikTokに投稿する

実作業時間目安: 4〜5時間。順番に：

1. [docs/account-setup.md](docs/account-setup.md) でアカウント開設（10分）
2. [episodes/ep01-hatomugi-workflow.md](episodes/ep01-hatomugi-workflow.md) を上から順にコピペ実行
3. 投稿完了

## ステータス

- [x] 企画・戦略の文書化
- [x] 1本目の台本確定
- [ ] キャラ画像生成（Gemini）
- [ ] 音声生成（CapCut）
- [ ] カット動画生成（Kling）
- [ ] リップシンク（Kling）
- [ ] Premiere Pro 仕上げ
- [ ] アカウント開設・初投稿
